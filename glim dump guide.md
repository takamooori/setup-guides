...
[3] 点群 → RViz          /debug_points（全点）と /debug_upper（上半球）を同時publish
                         RViz2側でトピックのチェックを切り替えて表示を選ぶ
                         フレーム切り替えあり（publish中にn/p/数字で変更可能）
[4] 全軌跡+マップ→RViz   /glim_path（軌跡）+ /glim_map（点群）を同時publish
[5] 距離分布確認         MAX_DIST選定のためのヒストグラム表示
```

### フレーム切り替え操作（[3]のpublish中）

```
現在: [ 21] 000021   [n]次  [p]前  [数字]直接指定  [q]終了
> n          <- 次のフレームへ
> p          <- 前のフレームへ
> 50         <- フレーム50に直接ジャンプ
> q          <- 終了してメニューに戻る
...
実行時に聞かれることはなく、スクリプト冒頭の2行だけ編集する：

```python
DEFAULT_DUMP = "~/ros2_ws/maps/dump_nakaniwa_0522"
MAX_DIST     = 50.0   # <- ここを変える
```

### RViz2の起動方法

[3][4]を選ぶとdumpディレクトリ内に`.rviz`ファイルが自動生成される。

```bash
# Inspector側でpublish開始後、別ターミナルで起動
rviz2 -d ~/ros2_ws/maps/dump_nakaniwa_0522/dump_nakaniwa_0522.rviz
```

RViz2のSave Configとは**完全に独立したファイル**なのでRViz2の既存設定を汚さない。
...
# 3. 距離分布確認（[5]）→ MAX_DISTの適切な値を決めてスクリプト冒頭を編集

# 4. 全軌跡表示（[4]）→ 走行パスに問題ないか目視確認

# 5. occlusion_analysis.py を実行（DUMP_DIRをスクリプト冒頭で変更してから）
python3 occlusion_analysis.py
```
